...
# Add background
add_bg("sss.jpeg")  # replace with your actual JPEG image file

# ========== LOAD MODEL ==========
model = load("linear_regression_model.joblib")  # make sure this file exists
...
# ========== INPUTS ==========
st.markdown(
    "<style>.glass {background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); "
    "border-radius: 20px; padding: 25px; box-shadow: 0 4px 20px rgba(0,0,0,0.2);}</style>",
...
with st.container():
    ...
    with col1:
        X1 = st.number_input(
            "🏗️ Relative Compactness", min_value=0.62, max_value=0.98, value=0.75
        )
        X2 = st.number_input(
            "📐 Surface Area", min_value=514.0, max_value=808.0, value=600.0
        )
        X3 = st.number_input(
            "🏠 Wall Area", min_value=294.0, max_value=416.0, value=350.0
        )
        X4 = st.number_input(
            "🏢 Roof Area", min_value=110.0, max_value=220.0, value=150.0
        )

    with col2:
        X5 = st.number_input(
            "📏 Overall Height", min_value=3.0, max_value=7.0, value=3.5
        )
        X6 = st.number_input(
            "🧭 Orientation", min_value=2, max_value=5, value=3
        )
        X7 = st.number_input(
            "🌞 Glazing Area", min_value=0.0, max_value=0.4, value=0.2
        )
        X8 = st.number_input(
